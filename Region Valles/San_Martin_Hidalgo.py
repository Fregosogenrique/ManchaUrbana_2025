"""
Comprehensive 20-Year Analysis of Urban Growth in San_Martin_Hidalgo, Jalisco, Mexico (2004-2024)
==================================================================================================

This script creates a comprehensive visualization of urban growth in San_Martin_Hidalgo over a
...
output_dir = 'San_Martin_Hidalgo_comparison'
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Set up a separate directory for individual images
individual_output_dir = 'San_Martin_Hidalgo_individual'
...
def export_to_geotiff(image, filename, region, description):
    ...
    task = ee.batch.Export.image.toDrive(
        ...
        folder='ManchaUrbana_San_Martin_Hidalgo',
...
def create_comparison_plot():
    ...
    """
    # Definir el punto central de San Martín Hidalgo, Jalisco
    san_martin_hidalgo_point = ee.Geometry.Point([-103.9284, 20.4347])

    # Crear un buffer de 7 km alrededor del punto central
    roi = san_martin_hidalgo_point.buffer(7000)  # 7000 metros de radio
    ...
    export_to_geotiff(landsat_2004, 'San_Martin_Hidalgo_2004_raw', roi, 'San_Martin_Hidalgo2004Raw')
    export_to_geotiff(landsat_2014, 'San_Martin_Hidalgo_2014_raw', roi, 'San_Martin_Hidalgo2014Raw')
    export_to_geotiff(landsat_2024, 'San_Martin_Hidalgo_2024_raw', roi, 'San_Martin_Hidalgo2024Raw')

    # Export visualized satellite images (for display)
    export_to_geotiff(vis_satellite_2004, 'San_Martin_Hidalgo_2004_rgb', roi, 'San_Martin_Hidalgo2004RGB')
    export_to_geotiff(vis_satellite_2014, 'San_Martin_Hidalgo_2014_rgb', roi, 'San_Martin_Hidalgo2014RGB')
    export_to_geotiff(vis_satellite_2024, 'San_Martin_Hidalgo_2024_rgb', roi, 'San_Martin_Hidalgo2024RGB')

    # Export urban change maps
    export_to_geotiff(urban_change_04_14, 'San_Martin_Hidalgo_change_2004_2014', roi, 'Change2004_2014')
    export_to_geotiff(urban_change_14_24, 'San_Martin_Hidalgo_change_2014_2024', roi, 'Change2014_2024')
    export_to_geotiff(urban_change_04_24, 'San_Martin_Hidalgo_change_2004_2024', roi, 'Change2004_2024')

    # Export urban masks for each year (for additional analysis)
    export_to_geotiff(urban_2004, 'San_Martin_Hidalgo_urban_2004', roi, 'Urban2004')
    export_to_geotiff(urban_2014, 'San_Martin_Hidalgo_urban_2014', roi, 'Urban2014')
    export_to_geotiff(urban_2024, 'San_Martin_Hidalgo_urban_2024', roi, 'Urban2024')

    print(
        "All GeoTIFF export tasks have been started. Files will be available in your Google Drive folder 'ManchaUrbana_San_Martin_Hidalgo'\n")
    ...
    img_2004 = display_ee_image(rgb_2004_url, axes[0, 0], 'San_Martin_Hidalgo 2004')
    img_2014 = display_ee_image(rgb_2014_url, axes[0, 1], 'San_Martin_Hidalgo 2014')
    img_2024 = display_ee_image(rgb_2024_url, axes[0, 2], 'San_Martin_Hidalgo 2024')
    ...
    plt.tight_layout()
    fig_title = fig.suptitle('Análisis de Crecimiento Urbano de San_Martin_Hidalgo, Jalisco (2004-2024)',
                             fontsize=16, y=1.02)
    plt.savefig(os.path.join(output_dir, 'San_Martin_Hidalgo_urban_comparison.png'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(output_dir, 'San_Martin_Hidalgo_urban_comparison.pdf'), bbox_inches='tight')

    print(
        f"Comprehensive 20-year urban growth analysis saved to {os.path.join(output_dir, 'San_Martin_Hidalgo_urban_comparison.png')}")
...
def main():
    ...
    print("Generating comprehensive 20-year urban growth analysis for San_Martin_Hidalgo...")
    try:
