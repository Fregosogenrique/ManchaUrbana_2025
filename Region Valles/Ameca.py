"""
Comprehensive 20-Year Analysis of Urban Growth in Ameca, Jalisco, Mexico (2004-2024)
==================================================================================================

This script creates a comprehensive visualization of urban growth in Ameca over a
...
output_dir = 'ameca_comparison'
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Set up a separate directory for individual images
individual_output_dir = 'ameca_individual'
...
def create_comparison_plot():
    ...
    """
    # Definir el punto central de Ameca, Jalisco
    ameca_point = ee.Geometry.Point([-104.04508, 20.54629])

    # Crear un buffer de 7 km alrededor del punto central
    roi = ameca_point.buffer(7000)  # 7000 metros de radio
    ...
    export_to_geotiff(landsat_2004, 'ameca_2004_raw', roi, 'Ameca004Raw')
    export_to_geotiff(landsat_2014, 'ameca_2014_raw', roi, 'Ameca2014Raw')
    export_to_geotiff(landsat_2024, 'ameca2024_raw', roi, 'Ameca2024Raw')

    # Export visualized satellite images (for display)
    export_to_geotiff(vis_satellite_2004, 'ameca_2004_rgb', roi, 'Ameca2004RGB')
    export_to_geotiff(vis_satellite_2014, 'ameca_2014_rgb', roi, 'Ameca2014RGB')
    export_to_geotiff(vis_satellite_2024, 'ameca2024_rgb', roi, 'Ameca2024RGB')

    # Export urban change maps
    export_to_geotiff(urban_change_04_14, 'ameca_change_2004_2014', roi, 'Change2004_2014')
    export_to_geotiff(urban_change_14_24, 'ameca_change_2014_2024', roi, 'Change2014_2024')
    export_to_geotiff(urban_change_04_24, 'ameca_change_2004_2024', roi, 'Change2004_2024')

    # Export urban masks for each year (for additional analysis)
    export_to_geotiff(urban_2004, 'ameca_urban_2004', roi, 'Urban2004')
    export_to_geotiff(urban_2014, 'ameca_urban_2014', roi, 'Urban2014')
    export_to_geotiff(urban_2024, 'ameca_urban_2024', roi, 'Urban2024')

    print(
        "All GeoTIFF export tasks have been started. Files will be available in your Google Drive folder 'ManchaUrbana_Ameca'\n")
    ...
    img_2004 = display_ee_image(rgb_2004_url, axes[0, 0], 'Ameca 2004')
    img_2014 = display_ee_image(rgb_2014_url, axes[0, 1], 'Ameca 2014')
    img_2024 = display_ee_image(rgb_2024_url, axes[0, 2], 'Ameca 2024')
    ...
    plt.tight_layout()
    fig_title = fig.suptitle('Análisis de Crecimiento Urbano de Ameca, Jalisco (2004-2024)',
                             fontsize=16, y=1.02)
    plt.savefig(os.path.join(output_dir, 'ameca_urban_comparison.png'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(output_dir, 'ameca_urban_comparison.pdf'), bbox_inches='tight')

    print(
        f"Comprehensive 20-year urban growth analysis saved to {os.path.join(output_dir, 'ameca_urban_comparison.png')}")
...
def main():
    ...
    print("Generating comprehensive 20-year urban growth analysis for Ameca...")
    try:
