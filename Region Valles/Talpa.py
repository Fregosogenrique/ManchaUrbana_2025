"""
Comprehensive 20-Year Analysis of Urban Growth in Talpa, Jalisco, Mexico (2004-2024)
==================================================================================================

This script creates a comprehensive visualization of urban growth in Talpa over a
...
output_dir = 'Talpa_comparison'
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Set up a separate directory for individual images
individual_output_dir = 'Talpa_individual'
...
def create_comparison_plot():
    ...
    """
    # Definir el punto central de Talpa de Allende, Jalisco
    talpa_point = ee.Geometry.Point([-104.8220, 20.3805])

    # Crear un buffer de 7 km alrededor del punto central
    roi = talpa_point.buffer(7000)  # 7000 metros de radio
    ...
    export_to_geotiff(landsat_2004, 'Talpa_2004_raw', roi, 'Talpa2004Raw')
    export_to_geotiff(landsat_2014, 'Talpa_2014_raw', roi, 'Talpa2014Raw')
    export_to_geotiff(landsat_2024, 'Talpa_2024_raw', roi, 'Talpa2024Raw')

    # Export visualized satellite images (for display)
    export_to_geotiff(vis_satellite_2004, 'Talpa_2004_rgb', roi, 'Talpa2004RGB')
    export_to_geotiff(vis_satellite_2014, 'Talpa_2014_rgb', roi, 'Talpa2014RGB')
    export_to_geotiff(vis_satellite_2024, 'Talpa_2024_rgb', roi, 'Talpa2024RGB')

    # Export urban change maps
    export_to_geotiff(urban_change_04_14, 'Talpa_change_2004_2014', roi, 'Change2004_2014')
    export_to_geotiff(urban_change_14_24, 'Talpa_change_2014_2024', roi, 'Change2014_2024')
    export_to_geotiff(urban_change_04_24, 'Talpa_change_2004_2024', roi, 'Change2004_2024')

    # Export urban masks for each year (for additional analysis)
    export_to_geotiff(urban_2004, 'Talpa_urban_2004', roi, 'Urban2004')
    export_to_geotiff(urban_2014, 'Talpa_urban_2014', roi, 'Urban2014')
    export_to_geotiff(urban_2024, 'Talpa_urban_2024', roi, 'Urban2024')

    print(
        "All GeoTIFF export tasks have been started. Files will be available in your Google Drive folder 'ManchaUrbana_Talpa'\n")
    ...
    img_2004 = display_ee_image(rgb_2004_url, axes[0, 0], 'Talpa 2004')
    img_2014 = display_ee_image(rgb_2014_url, axes[0, 1], 'Talpa 2014')
    img_2024 = display_ee_image(rgb_2024_url, axes[0, 2], 'Talpa 2024')
    ...
    plt.tight_layout()
    fig_title = fig.suptitle('Análisis de Crecimiento Urbano de Talpa, Jalisco (2004-2024)',
                             fontsize=16, y=1.02)
    plt.savefig(os.path.join(output_dir, 'Talpa_urban_comparison.png'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(output_dir, 'Talpa_urban_comparison.pdf'), bbox_inches='tight')

    print(
        f"Comprehensive 20-year urban growth analysis saved to {os.path.join(output_dir, 'Talpa_urban_comparison.png')}")
...
def main():
    ...
    print("Generating comprehensive 20-year urban growth analysis for Talpa...")
    try:
