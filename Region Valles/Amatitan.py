...
This script creates a comprehensive visualization of urban growth in Amatitan over a
...
output_dir = 'amatitan_comparison'
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Set up a separate directory for individual images
individual_output_dir = 'amatitan_individual'
...
def create_comparison_plot():
    ...
    """
    # Get the Region of Interest (ROI) - Amatitan, Jalisco coordinates
    amatitan_point = ee.Geometry.Point([-103.73, 20.83])
    roi = amatitan_point.buffer(7000)  # 7km buffer around center, adjusted for smaller town size
    ...
    export_to_geotiff(landsat_2004, 'amatitan_2004_raw', roi, 'Amatitan2004Raw')
    export_to_geotiff(landsat_2014, 'amatitan_2014_raw', roi, 'Amatitan2014Raw')
    export_to_geotiff(landsat_2024, 'amatitan_2024_raw', roi, 'Amatitan2024Raw')
    
    # Export visualized satellite images (for display)
    export_to_geotiff(vis_satellite_2004, 'amatitan_2004_rgb', roi, 'Amatitan2004RGB')
    export_to_geotiff(vis_satellite_2014, 'amatitan_2014_rgb', roi, 'Amatitan2014RGB')
    export_to_geotiff(vis_satellite_2024, 'amatitan_2024_rgb', roi, 'Amatitan2024RGB')
    
    # Export urban change maps
    export_to_geotiff(urban_change_04_14, 'amatitan_change_2004_2014', roi, 'Change2004_2014')
    export_to_geotiff(urban_change_14_24, 'amatitan_change_2014_2024', roi, 'Change2014_2024')
    export_to_geotiff(urban_change_04_24, 'amatitan_change_2004_2024', roi, 'Change2004_2024')
    
    # Export urban masks for each year (for additional analysis)
    export_to_geotiff(urban_2004, 'amatitan_urban_2004', roi, 'Urban2004')
    export_to_geotiff(urban_2014, 'amatitan_urban_2014', roi, 'Urban2014')
    export_to_geotiff(urban_2024, 'amatitan_urban_2024', roi, 'Urban2024')
    
    print("All GeoTIFF export tasks have been started. Files will be available in your Google Drive folder 'ManchaUrbana_Amatitan'\n")
    ...
    img_2004 = display_ee_image(rgb_2004_url, axes[0, 0], 'Amatitan 2004')
    img_2014 = display_ee_image(rgb_2014_url, axes[0, 1], 'Amatitan 2014')
    img_2024 = display_ee_image(rgb_2024_url, axes[0, 2], 'Amatitan 2024')
    ...
    plt.tight_layout()
    fig_title = fig.suptitle('Análisis de Crecimiento Urbano de Amatitan, Jalisco (2004-2024)',
                             fontsize=16, y=1.02)
    plt.savefig(os.path.join(output_dir, 'amatitan_urban_comparison.png'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(output_dir, 'amatitan_urban_comparison.pdf'), bbox_inches='tight')

    print(
        f"Comprehensive 20-year urban growth analysis saved to {os.path.join(output_dir, 'amatitan_urban_comparison.png')}")
...
def main():
    ...
    print("Generating comprehensive 20-year urban growth analysis for Amatitan...")
    try:
