"""
Comprehensive 20-Year Analysis of Urban Growth in Hostotipaquillo, Jalisco, Mexico (2004-2024)
==================================================================================================

This script creates a comprehensive visualization of urban growth in Hostotipaquillo over a
...
output_dir = 'Hostotipaquillo_comparison'
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Set up a separate directory for individual images
individual_output_dir = 'Hostotipaquillo_individual'
...
def export_to_geotiff(image, filename, region, description):
    ...
    task = ee.batch.Export.image.toDrive(
        ...
        folder='ManchaUrbana_Hostotipaquillo',
...
def create_comparison_plot():
    ...
    """
    # Definir el punto central de Hostotipaquillo, Jalisco
    hostotipaquillo_point = ee.Geometry.Point([-104.0506, 21.0585])

    # Crear un buffer de 7 km alrededor del punto central
    roi = hostotipaquillo_point.buffer(7000)  # 7000 metros de radio
    ...
    export_to_geotiff(landsat_2004, 'Hostotipaquillo_2004_raw', roi, 'Hostotipaquillo2004Raw')
    export_to_geotiff(landsat_2014, 'Hostotipaquillo_2014_raw', roi, 'Hostotipaquillo2014Raw')
    export_to_geotiff(landsat_2024, 'Hostotipaquillo_2024_raw', roi, 'Hostotipaquillo2024Raw')

    # Export visualized satellite images (for display)
    export_to_geotiff(vis_satellite_2004, 'Hostotipaquillo_2004_rgb', roi, 'Hostotipaquillo2004RGB')
    export_to_geotiff(vis_satellite_2014, 'Hostotipaquillo_2014_rgb', roi, 'Hostotipaquillo2014RGB')
    export_to_geotiff(vis_satellite_2024, 'Hostotipaquillo_2024_rgb', roi, 'Hostotipaquillo2024RGB')

    # Export urban change maps
    export_to_geotiff(urban_change_04_14, 'Hostotipaquillo_change_2004_2014', roi, 'Change2004_2014')
    export_to_geotiff(urban_change_14_24, 'Hostotipaquillo_change_2014_2024', roi, 'Change2014_2024')
    export_to_geotiff(urban_change_04_24, 'Hostotipaquillo_change_2004_2024', roi, 'Change2004_2024')

    # Export urban masks for each year (for additional analysis)
    export_to_geotiff(urban_2004, 'Hostotipaquillo_urban_2004', roi, 'Urban2004')
    export_to_geotiff(urban_2014, 'Hostotipaquillo_urban_2014', roi, 'Urban2014')
    export_to_geotiff(urban_2024, 'Hostotipaquillo_urban_2024', roi, 'Urban2024')

    print(
        "All GeoTIFF export tasks have been started. Files will be available in your Google Drive folder 'ManchaUrbana_Hostotipaquillo'\n")
    ...
    img_2004 = display_ee_image(rgb_2004_url, axes[0, 0], 'Hostotipaquillo 2004')
    img_2014 = display_ee_image(rgb_2014_url, axes[0, 1], 'Hostotipaquillo 2014')
    img_2024 = display_ee_image(rgb_2024_url, axes[0, 2], 'Hostotipaquillo 2024')
    ...
    plt.tight_layout()
    fig_title = fig.suptitle('Análisis de Crecimiento Urbano de Hostotipaquillo, Jalisco (2004-2024)',
                             fontsize=16, y=1.02)
    plt.savefig(os.path.join(output_dir, 'Hostotipaquillo_urban_comparison.png'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(output_dir, 'Hostotipaquillo_urban_comparison.pdf'), bbox_inches='tight')

    print(
        f"Comprehensive 20-year urban growth analysis saved to {os.path.join(output_dir, 'Hostotipaquillo_urban_comparison.png')}")
...
def main():
    ...
    print("Generating comprehensive 20-year urban growth analysis for Hostotipaquillo...")
    try:
